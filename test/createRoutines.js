...
describe('create a bunch of routines with `createRoutines`', () => { 
  ...
  const SCHEME_PLUS = {
    ...
    customMeta: {
      '+TRIGGER': [null, () => ({ some: 'data' })],
      '+REQUEST': [null, () => ({ some: 'data' })],
      '+SUCCESS': [null, () => ({ some: 'data' })],
      '+FAILURE': [null, () => ({ some: 'data' })],
      '+FULFILL': [null, () => ({ some: 'data' })]
    },
    additionalStages: {
      '+ANY': null,
      '+CUSTOM_PAYLOAD': [
  ...
  const routines_plus = createRoutines(SCHEME);
  ...
  it('should create default routine by keys with plus', () => {
    const routine = routines_plus.default;
